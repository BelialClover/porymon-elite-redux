...
''')
    ...
    speciesName = f'{pokemon_data.species.title().split("_", 1)[0]}'
    formName = f'{pokemon_data.species.title().split("_", 1)[1]}'

    editFiles(pory_path, version_path, config, module)
    copyAssets(join(pory_path, version_path, species), join(config.pokeemerald_path, "graphics", "pokemon", str(speciesName).lower(), str(formName).lower()), assets)
    writeBackAll()
...
''')
    ...
    while True:
        ...
        if user_input == 'y':
            ...
            restoreAll()
            shutil.rmtree(join(config.pokeemerald_path, "graphics", "pokemon", str(speciesName).lower(), str(formName).lower()))
            sys.exit()
        elif user_input == 'n':
            sys.exit()
        else:
...
def editFiles(pory_path, version_path, config, module):
    ...
    # Header Files
    # Species definitions
    species_header = module.SpeciesFormH(join(config.pokeemerald_path, include_constants, "species.h"))
    ...
    pokemon_header = module.PokemonFormH(join(config.pokeemerald_path, "src", "data", "graphics", "pokemon.h"))
    pokemon_header.appendData(speciesName, formName, species_header.prevSpecies)
    ...
    # Source Files
    pokemon_source = module.PokemonCForms(join(config.pokeemerald_path, "src", "pokemon.c"))
